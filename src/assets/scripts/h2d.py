...
class HtmlToDocx(HTMLParser):
    ...
    def handle_img(self, current_attrs):
        ...
        # check if image starts with data:.*base64, (custom code)
        if image and image.startswith('data:image/'):
            ...
            # extract the width and height of the image if they exist (convert the width and height to inches)
            width = None
            height = None
            if 'width' in current_attrs:
                width = float(current_attrs['width'])
                width = Inches(width / 96)
            if 'height' in current_attrs:
                height = float(current_attrs['height'])
        ...
        if image:
            try:
                if isinstance(self.doc, docx.document.Document):
                    # Run created first so the image is not added on a new line
